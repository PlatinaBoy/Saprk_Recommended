...
object test {
  def main(args: Array[String]): Unit = {
    ...
    val movieAvgRate = sparkSession.sql("select movieid,round(avg(rate),1) as avg_rate  from mite8.mite_ratings group by movieid")
      .rdd.map {
      f =>
        (f.get(0), f.get(1))
    }
    ...
    //获取电影的基本属性数据
    val moviesData = sparkSession
      .sql("select movieid,title,genre from m    ite8.mite_movies").rdd

    //获取电影tags数据
    val tagsData = sparkSession
      .sql("select movieid,tag from mite8.mite_tags").rdd
